...
import { createSlug, posterURL } from './utils'
...
export async function addMovieAction(movieId: number) {
  ...
  const movidDataReq = await fetch(
    `https://api.themoviedb.org/3/movie/${movieId}?language=en-US&api_key=${process.env.TMDB_API_KEY}`,
  )
  const { title, poster_path, overview, tagline, genres: genreObjects } = await movidDataReq.json()
